...
# نسبة الفائدة (0% حالياً)
LOAN_INTEREST_RATE = 0.0 

# تعريف فئات القروض
LOAN_CATEGORIES = {
    "loan1000": {"name": "قرض الـ 1000", "amount": 1000, "min_req": 250, "deduct": 250},
    "loan5000": {"name": "قرض الـ 5000", "amount": 5000, "min_req": 1000, "deduct": 1000},
    "loan10000": {"name": "قرض الـ 10000", "amount": 10000, "min_req": 2500, "deduct": 2500}
}

# رفع الإعدادات للفايربيس إذا لم تكن موجودة
if not fb_get("loan_settings"):
    fb_patch("loan_settings", LOAN_CATEGORIES)
def auto_reward_task():
    ...
    # --- جزء استقطاع القروض ---
    all_loans = fb_get("user_loans")
    if all_loans:
        today = datetime.now().strftime('%Y-%m-%d')
        for l_uid, loans in all_loans.items():
            for lid, ldata in loans.items():
                if ldata['next_date'] <= today:
                    u_info = fb_get(f"users/{l_uid}")
                    pts = u_info.get('points', 0)
                    
                    deduct_amt = ldata['deduct']
                    if ldata['remaining'] < deduct_amt:
                        deduct_amt = ldata['remaining']
                        
                    new_rem = ldata['remaining'] - deduct_amt
                    new_pts = pts - deduct_amt
                    
                    # تحديث البيانات
                    fb_patch(f"users/{l_uid}", {"points": new_pts})
                    
                    if new_rem <= 0:
                        requests.delete(f"{FIREBASE_URL}/user_loans/{l_uid}/{lid}.json?auth={DATABASE_SECRET}")
                        bot.send_message(l_uid, f"🎊 مبروك! تم الانتهاء من تسديد {ldata['name']} بالكامل.")
                        bot.send_message(CH_USER, f"🏁 المستخدم {l_uid} سدد قرضه بالكامل.")
                    else:
                        next_m = (datetime.now() + timedelta(days=30)).strftime('%Y-%m-%d')
                        fb_patch(f"user_loans/{l_uid}/{lid}", {"remaining": new_rem, "next_date": next_m})
                        
                        msg = f"📉 **استقطاع القرض الدوري**\n🏷 {ldata['name']}\n💸 خصم: {deduct_amt}\n💰 المتبقي: {new_rem}\n📅 الموعد القادم: {next_m}"
                        bot.send_message(l_uid, msg)
                        bot.send_message(CH_USER, f"📉 استقطاع {deduct_amt} من {l_uid} (قرض).")                
...
def main_markup():
    ...
    m.add(
        ...
        types.InlineKeyboardButton("📋 قروضاتي", callback_data="my_loans"),
        types.InlineKeyboardButton("القروض", callback_data="loans_menu"),
        types.InlineKeyboardButton("🏆 لوحة المتصدرين", callback_data="leaderboard_home"),
        ...
        types.InlineKeyboardButton("📜 الضمانات وسياسة الاستخدام", callback_data="policy_btn")
    )
    return m
...
✅ تم تحديث عدادك وتمديد الصلاحية.
    ...
    elif call.data == "leaderboard_home":
        text = "🏆 <b>مرحباً بك في لوحة المتصدرين</b>\n\nاختر التصنيف الذي تريد استعراضه:"
        m = types.InlineKeyboardMarkup(row_width=1)
        m.add(
            types.InlineKeyboardButton("💰 المتصدرين بالنقاط", callback_data="top_points"),
            types.InlineKeyboardButton("👥 المتصدرين بالدعوات", callback_data="top_refs"),
            types.InlineKeyboardButton("📊 المتصدرين بالعدادات", callback_data="top_counters"),
            types.InlineKeyboardButton("🔙 رجوع", callback_data="back_main")
        )
        bot.edit_message_text(text, uid, call.message.id, reply_markup=m, parse_mode="HTML")

    elif call.data.startswith("top_"):
        category = call.data.split("_")[1] # points, refs, or counters
        all_users = fb_get("users") or {}
        
        # تحويل البيانات إلى قائمة لسهولة الترتيب
        users_list = []
        for u_id, u_data in all_users.items():
            # تحديد القيمة حسب القسم
            if category == "points":
                val = u_data.get('points', 0)
                label = "نقطة"
                title = "💰 متصدري النقاط"
            elif category == "refs":
                val = u_data.get('referrals', 0)
                label = "دعوة"
                title = "👥 متصدري الدعوات"
            else: # counters
                val = u_data.get('counter', 0)
                label = "سرعة"
                title = "📊 متصدري العدادات"
            
            users_list.append({"id": u_id, "val": val})

        # ترتيب القائمة من الأعلى للأقل وأخذ أول 10
        top_10 = sorted(users_list, key=lambda x: x['val'], reverse=True)[:10]

        text = f"<b>🏆 {title} (Top 10)</b>\n\n"
        text += "الترتيب | الآيدي | الكمية\n"
        text += "━━━━━━━━━━━━━━\n"
        
        for i, user in enumerate(top_10, 1):
            # تمييز الثلاثة الأوائل برموز
            medal = {1: "🥇", 2: "🥈", 3: "🥉"}.get(i, f"<code>{i}</code>")
            text += f"{medal} | <code>{user['id']}</code> | <b>{user['val']:,}</b> {label}\n"

        m = types.InlineKeyboardMarkup()
        m.add(types.InlineKeyboardButton("🔙 رجوع للوحة", callback_data="leaderboard_home"))
        bot.edit_message_text(text, uid, call.message.id, reply_markup=m, parse_mode="HTML")
        ...
        if can_claim:
            ...
            m.add(types.InlineKeyboardButton("🔙 رجوع", callback_data="back_main"))
            bot.edit_message_text(text, uid, call.message.id, reply_markup=m)
    elif call.data == "my_loans":
        user_loans = fb_get(f"user_loans/{uid}")
        
        if not user_loans:
            m = types.InlineKeyboardMarkup()
            m.add(types.InlineKeyboardButton("🔙 رجوع", callback_data="back_main"))
            return bot.edit_message_text("⚠️ **ليس لديك أي قروض نشطة حالياً.**", uid, call.message.id, reply_markup=m, parse_mode="HTML")
        
        text = "📋 قائمة قروضك النشطة:\n\n"
        for lid, ldata in user_loans.items():
            text += f"━━━━━━━━━━━━━━\n"
            text += f"🏷 النوع: {ldata['name']}\n"
            text += f"💰 الإجمالي: {ldata['total']} نقطة\n"
            text += f"📉 المتبقي للتسديد: {ldata['remaining']} نقطة\n"
            text += f"💸 القسط الشهري: {ldata['deduct']}\n"
            text += f"📅 موعد الاستقطاع القادم: {ldata['next_date']}\n"
            text += f"🗓 تاريخ البدء: {ldata['start_date']}\n"
        ...
        m.add(types.InlineKeyboardButton("🔙 رجوع", callback_data="back_main"))
        bot.edit_message_text(text, uid, call.message.id, reply_markup=m, parse_mode="HTML")
    # القائمة الرئيسية للقروض
    elif call.data == "loans_menu":
        u = fb_get(f"users/{uid}") or {}
        user_counter = u.get('counter', 0)
        user_points = u.get('points', 0)
        
        m = types.InlineKeyboardMarkup(row_width=1)
        # جلب القروض من الفايربيس للتحكم بها لاحقاً
        loans = fb_get("loan_settings") or LOAN_CATEGORIES
        
        for lid, info in loans.items():
            m.add(types.InlineKeyboardButton(f"💰 {info['name']} | عداد {info['min_req']}+", callback_data=f"l_info_{lid}"))
        
        m.add(types.InlineKeyboardButton("🔙 رجوع", callback_data="back_main"))
        
        # تنسيق الرسالة بشكل مرتب
        text = (
            f"🏦 <b>مرحباً بك في نظام القروض</b>\n\n"
            f"حقوقك الحالية:\n"
            f"📊 قيمة عدادك: <code>{user_counter}</code>\n"
            f"💎 رصيد نقاطك: <code>{user_points}</code>\n\n"
            f"⚠️ <i>اختر الفئة التي تناسب عدادك واحتياجك من الأسفل:</i>"
        )
        
        bot.edit_message_text(text, uid, call.message.id, reply_markup=m, parse_mode="HTML")
    # عرض تفاصيل القرض المختار
    elif call.data.startswith("l_info_"):
        lid = call.data.split("_")[2]
        loan = fb_get(f"loan_settings/{lid}")
        u = fb_get(f"users/{uid}")
        
        user_counter = u.get('counter', 0)
        interest = loan['amount'] * LOAN_INTEREST_RATE
        total_pay = int(loan['amount'] + interest)
        
        text = f"📋 **تفاصيل {loan['name']}**\n\n"
        text += f"🔹 مبلغ القرض: {loan['amount']} نقطة\n"
        text += f"➕ نسبة الفائدة: {LOAN_INTEREST_RATE}%\n"
        text += f"💰 المجموع للتسديد: {total_pay}\n"
        text += f"📉 استقطاع شهري: {loan['deduct']} نقطة\n"
        text += f"⚠️ العداد المطلوب: {loan['min_req']}\n"
        text += f"━━━━━━━━━━━━━━\n"

        m = types.InlineKeyboardMarkup()
        if user_counter >= loan['min_req']:
            text += "✅ عدادك يسمح لك بالاقتراض."
            m.add(types.InlineKeyboardButton("✅ اقتراض الآن", callback_data=f"l_buy_{lid}"))
        else:
            text += f"❌ لا يمكنك الاقتراض. عدادك حالياً ({user_counter}) وتحتاج ({loan['min_req'] - user_counter}) إضافية."
        
        m.add(types.InlineKeyboardButton("🔙 رجوع", callback_data="loans_menu"))
        bot.edit_message_text(text, uid, call.message.id, reply_markup=m, parse_mode="HTML")

    # تأفيذ عملية الاقتراض
    elif call.data.startswith("l_buy_"):
        lid = call.data.split("_")[2]
        loan = fb_get(f"loan_settings/{lid}")
        u = fb_get(f"users/{uid}")
        ...
        # تحديث رصيد المستخدم وإضافة القرض
        fb_patch(f"users/{uid}", {"points": u.get('points', 0) + loan['amount']})
        fb_patch(f"user_loans/{uid}/{loan_id}", loan_data)
        
        # نشر في القناة
        ch_text = f"⚠️ **عملية قرض جديدة**\n👤 المستخدم: <code>{uid}</code>\n💰 المبلغ: {loan['amount']}\n📉 التسديد: {loan['deduct']} شهرياً"
        bot.send_message(CH_USER, ch_text, parse_mode="HTML")
        
        bot.answer_callback_query(call.id, "✅ تم منحك القرض بنجاح!", show_alert=True)
        bot.delete_message(uid, call.message.id)
        start(call) # الرجوع للقائمة الرئيسية
    elif call.data == "policy_btn":
        # جلب تاريخ ووقت بغداد الحالي بنظام 12 ساعة
        # ملاحظة: timedelta(hours=3) لضبط توقيت بغداد (UTC+3)
        now_baghdad = datetime.utcnow() + timedelta(hours=3)
        current_time = now_baghdad.strftime('%Y-%m-%d %I:%M %p')
        
        # جلب عدد المستخدمين من الفايربيس
        all_users = fb_get("users")
        user_count = len(all_users) if all_users else 0
        
        # نص الرسالة منسق بشكل احترافي
        text = f"<b>📜 الضمانات وسياسة استخدام {BOT_NAME}</b>\n\n"
        text += f"<blockquote>نحن نضمن لك تجربة آمنة وشفافة، جميع العمليات داخل البوت مراقبة ومحمية لضمان حقوق المستخدمين.</blockquote>\n\n"
        
        text += f"<b>⚖️ القوانين والشروط:</b>\n"
        text += f"• يمنع استخدام الحسابات الوهمية أو محاولة التلاعب بالعدادات.\n"
        text += f"• أي محاولة غش تؤدي لحظر حسابك نهائياً وحذف نقاطك.\n"
        text += f"• البوت غير مسؤول عن ضياع بياناتك في حال مخالفة الشروط.\n\n"
        
        text += f"<b>📊 إحصائيات سريعة:</b>\n"
        text += f"👥 عدد مستخدمي البوت: <code>{user_count}</code>\n"
        text += f"⏰ الوقت الحالي (بغداد): <code>{current_time}</code>\n\n"
        
        text += f"<i>تأكد دائماً من متابعة قناة الإثباتات لرؤية عمليات السحب الموثقة.</i>"

        m = types.InlineKeyboardMarkup(row_width=2)
        btn_channel = types.InlineKeyboardButton("📢 قناة البوت", url="https://t.me/my00002") # استبدل الرابط
        btn_owner = types.InlineKeyboardButton("👨‍💻 المالك", url=f"https://t.me/{ADMIN_USER.replace('@','')}")
        btn_proofs = types.InlineKeyboardButton("✅ اثباتات السحب", url="https://t.me/my00002") # استبدل الرابط
        back = types.InlineKeyboardButton("🔙 رجوع", callback_data="back_main")
        
        m.add(btn_channel, btn_owner)
        m.add(btn_proofs)
        m.add(back)
